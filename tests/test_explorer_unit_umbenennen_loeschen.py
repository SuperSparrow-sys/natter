...
def test_explorer_zeigt_einen_knopf_fuer_jede_unit(tmp_path: Path) -> None:
    ...
    projekt = fenster.projekt_oeffnen(_projekt_kopie(tmp_path) / "ampel.natter")
...
def test_unit_umbenennen(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    fenster = HauptFenster()
    projekt = fenster.projekt_oeffnen(_projekt_kopie(tmp_path) / "ampel.natter")
    unit_pfad = next(p for p in projekt.units() if p.stem == "u_ampel")
    ...
    monkeypatch.setattr(
        ...
        staticmethod(lambda *a, **k: ("u_ampel_neu.py", True)),
    )

    fenster._unit_umbenennen(unit_pfad)

    neuer_pfad = unit_pfad.parent / "u_ampel_neu.py"
    assert neuer_pfad.exists()
    assert not unit_pfad.exists()
    assert any(p.name == "u_ampel_neu.py" for p in fenster.projekt.units())
...
def test_unit_umbenennen_haelt_offenen_tab_synchron(
    ...
    assert fenster.editor_tabs.tabText(index) == "u_ampel_neu.py"
